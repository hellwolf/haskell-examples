...
),c+1)(x+m,y+n)q}in s==0?(e==0?f 1$k)$s==1?(r&&e==o!!w?f 2$k)$not r||e==0?k$e==w
?(a,c+1)$f 2;q a d|((b,w),c)<-a%d=(c==0?b$(w^l d$b),o!!w);z h a@(b,w)=print(n!!w
,(`c`b)&[0..2])>>t b s>>(w/=h?z h(e a)$getLine>>=z h.q a.read);e a=q a.snd.f(\
c@(w,_)e@(d,_)->d>w?e$c)(0,(0,0))$((,)=<<snd.(a%))&k r};z 1(1^35$1^28$2^27$2^36$
replicate 64 0,1)}
-- ^10 ------------------------------------------------------------------ 80> --
