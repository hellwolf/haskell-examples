main=do{let{i True x _=x;i _ _ y=y;k=(<*>)=<<((,)<$>);m=map;f=foldl;r=[0..7];j x
=x>=0&&x<=7;l(x,y)=8*y+x;o 1=2;o _=1;c=(length.).filter.(==);g w i b=take i b++w
:drop (i+1)b;s=m show r;u=(putStrLn.).f(++);n 0="_";n 1="X";n 2="O";v k(a@(b,w),
c)p@(x,y)q@(m,n)s=let{r=j x&&j y;t=l p;e=b!!t;f=v k((g w t b,w),c+1)(x+m,y+n)q;h
|s==0=i(e==0)(f 1)k|s==1=i(r&&e==o w)(f 2)k|s==2=i(not r||e==0)k(i(e==w)(a,c+1)(
f 2))}in h;p a c=f(\k d->v k k c d 0)(a,0)(k[-1..1]);y([],_)_=u" "s;y(a,b)k=u(k
!!0)(m n a)>>t b(drop 1k);q a d=let((b,w),c)=p a d in(i(c==0)b(g w(l d)b),o w);z
h a@(b,w)=print(n w,m(`c`b)[0..2])>>t b s>>i(w==h)(getLine>>=z h.q a.read)(z h(e
a));t=y.splitAt 8;e a=q a.snd.f(\c@(w,_)e@(d,_)->i(d>w)e c)(0,(0,0)).m((,)=<<snd
.p a)$k r};z 1(g 1 35.g 1 28.g 2 27.g 2 36$replicate 64 0,1)}
-- ^10 ------------------------------------------------------------------ 80> --
